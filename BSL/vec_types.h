...
namespace BSL
{
    // Only valid for arithmetic types and for vectors holding 2-4 elements
    template<typename T, unsigned N, class = typename std::enable_if<std::is_arithmetic<T>::value && (N > 1 && N <= 4)>>
    class VecT
    {
    public:
        VecT()
    ...
    private:
        ...
        constexpr u32 GetSizeBytes() const
        {
            return sizeof(T) * N;
        }
    };
...
namespace std
{
	DEFINE_HASH_2D_VEC(BSL::Vec2f, float);
	DEFINE_HASH_2D_VEC(BSL::Vec2i, i32);
	DEFINE_HASH_2D_VEC(BSL::Vec2u, u32);
	DEFINE_HASH_3D_VEC(BSL::Vec3f, float);
	DEFINE_HASH_3D_VEC(BSL::Vec3i, i32);
	DEFINE_HASH_3D_VEC(BSL::Vec3u, u32);
	DEFINE_HASH_4D_VEC(BSL::Vec4f, float);
	DEFINE_HASH_4D_VEC(BSL::Vec4i, i32);
	DEFINE_HASH_4D_VEC(BSL::Vec4u, u32);
}
